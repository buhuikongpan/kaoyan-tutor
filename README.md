...
2. **学习**：选视频播放（自动**从上次播放位置续播**）→ 切换 Agent：
   ...
   - **C 课后问答**：自由提问
3. **🎤 语音输入（实时草稿）**：输入框左侧麦克风按钮 → 说话时**边说边出字**（每 2.5s 自动刷新草稿）→ 再点停止定稿，改完回车发送。引擎在设置页**独立配置**（与视频字幕解耦）：
   - `自动`（默认）：有千问 Key 走云（qwen3-asr-flash **免费模型，电脑零负担**），否则本地 Whisper
   - `本地 Whisper`：免费离线；`千问云`：需填 Key，免费模型
